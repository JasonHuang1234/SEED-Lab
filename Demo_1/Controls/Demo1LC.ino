...
const float R = 0.075;              // Wheel radius in meters
const float wheel_base = 0.355;    // Distance between wheels in meters (0.3556)
...
float Kp_vel[2] = {3,3.2};
float Kp_pos[2] = {4,4};
float Ki_pos[2] = {0.05,0.05};
...
float angle_position_0 = actual_pos[0];
float angle_position_1 = actual_pos[1];
...
void loop() {
    ...
    if (received){ // Code to recieve instructions from pi
    noInterrupts();
    ...
    if (current_time >= 5) {
      if (abs(desired_pos_xy[0]) < abs(ANGLE)) {
        desired_pos_xy[0] += constrain(ANGLE*0.005,-0.36,0.36); // Slowly increase desired position to prevent high acceleration
        desired_pos_xy[0] = constrain(desired_pos_xy[0],-abs(ANGLE),abs(ANGLE));
        angle_changed = true;
        angle_set = false;
      }
      
      if (desired_pos_xy[1] < DISTANCE*(0.3048) && angle_set && (current_time > (angle_set_time + 3) ) ) {
        desired_pos_xy[1] += DISTANCE*(0.3048)*0.001; // Slowly increase desired position to prevent high acceleration
        desired_pos_xy[1] = constrain(desired_pos_xy[1],0,DISTANCE*(0.3048));
    ...
    if (!angle_set) { // If the robot has not moved to the desired angle
      ...
      if ( (abs(phi - (desired_pos_xy[0]*(pi/180))) < ANGLE_TOLERANCE) && actual_vel[0] <  0.1) {
        ...
        angle_position_0 = actual_pos[0];
        angle_position_1 = actual_pos[1];

        angle_set_time = millis() / 1000;
      }

    } else if (angle_set) { 

      // calculations to convert desired robot distance to desired wheel angle
      desired_pos_r[0] = (desired_pos_xy[1] / R) + angle_position_0;
      desired_pos_r[1] = (desired_pos_xy[1] / R) + angle_position_1;
    ...
    for(int i=0;i<2;i++) {
      ...
      integral_error[i] = constrain(integral_error[i], -50.0, 50.0); // Constrain integral error

      // Calculate desired velocity for each motor
      desired_vel[i] = Kp_pos[i] * pos_error[i] + Ki_pos[i] * integral_error[i]; // Constrain maximum desired veolcity to prevent slipping?

      // Add error to correct for drifting
      float heading_error = desired_pos_xy[0]*(pi/180) - phi;
      float heading_correction = 5 * heading_error;
      desired_vel[0] -= heading_correction;
      desired_vel[1] += heading_correction;

      // Calculate velocity error and applied voltage
      vel_error[i] = desired_vel[i] - actual_vel[i];
      applied_voltage[i] = Kp_vel[i]*vel_error[i];

      // Calculate PWM signal 
      PWM[0] = constrain( (abs(applied_voltage[0]) / BATTERY_VOLTAGE) * 255, 0, 180); // 180
      PWM[1] = constrain( (abs(applied_voltage[1]) / BATTERY_VOLTAGE) * 255, 0, 189); // 189
